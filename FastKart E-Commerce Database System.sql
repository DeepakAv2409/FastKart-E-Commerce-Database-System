...
select EmailId, count(*) as Total_Transactions from purchasedetails group by EmailId having count(*)>10;


-- 3. List the Total QuantityAvailable category wise in descending order. (Name of the category, QuantityAvailable) 
select CategoryName as 'Name of the category', sum(QuantityAvailable) TotalQuantityAvailable from products p left join categories c on p.categoryId=c.categoryId group by CategoryName;
...
select pd.ProductId, ProductName, categoryName, sum(QuantityPurchased) Total_Purchased_Quantity from purchasedetails pd inner join products p on pd.ProductId=p.ProductId inner join categories c on p.categoryId = c.categoryId group by ProductId, ProductName, c.CategoryName order by Total_Purchased_Quantity desc limit 1;


-- 5. Display the number of male and female customers in fastkart. 
select gender, count(*) from users u inner join roles r on u.roleId = r.roleId where u.roleId = 2 group by gender;
...
select ProductId, ProductName, Price, IF(Price<2000, 'Affordable', IF(Price>2000 and Price<50000, 'High End Stuff', 'Luxury')) Item_Classes from products;


-- 7. Write a query to display Productld, ProductName, CategoryName, Old Price(price) and New Price as per the following criteria 
...
select ProductId, ProductName, CategoryName, Price as Old_Price, IF(CategoryName='Motors', Price-3000, IF(CategoryName='Electronics', Price+50, IF(CategoryName='Fashion', Price+150, Price))) New_Price from products p inner join categories c on p.categoryId = c.categoryId;
...
select concat(round(count(*)/(select count(*) from users)*100,2), '%') as 'Percentage of female' from users where gender='F';


-- 9. Display the average balance for both card types for those records only where CVNumber > 333 and NameOnCard ends with the alphabet "e".
select cardType, round(avg(balance),2) from carddetails where CVVNumber>333 and NameOnCard like '%e' group by cardType;
...
select ProductName, CategoryName, Price*QuantityAvailable value from products p left join categories c on p.categoryId = c.categoryId where CategoryName!='Motors' and Price*QuantityAvailable<(select max(Price*QuantityAvailable) from products p left join categories c on p.categoryId = c.categoryId where CategoryName!='Motors') order by value desc limit 1;
